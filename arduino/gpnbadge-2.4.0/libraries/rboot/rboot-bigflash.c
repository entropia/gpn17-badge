...
void ICACHE_RAM_ATTR Cache_Read_Enable_New(void) {

	if (rBoot_mmap_1 == 0xff) {
		uint32 val;
		rboot_config conf;

                sdkChipSize = flashchip->chip_size;
		flashchip->chip_size = 8 * 1024 * 1024; //changing the sdk flashsize at boot

		SPIRead(BOOT_CONFIG_SECTOR * SECTOR_SIZE, &conf, sizeof(rboot_config));
		//flashchip->chip_size = sdkChipSize;
